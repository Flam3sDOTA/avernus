function OnStartTouch(trigger)
	...
	if activator:IsRealHero() then
		for i=0, 8 do
			...
			if item and item:GetAbilityName() == "item_gate_3_key" then
				local gate3key = item
				...
				for _, enemy in pairs(enemies) do
					if enemy:GetUnitName() == "npc_dota_gate_3" then
						...
						UTIL_Remove( enemy )
						local hAnimGate = CreateUnitByName( "npc_dota_gate_3_anim", vPos, false, nil, nil, DOTA_TEAM_BADGUYS )
